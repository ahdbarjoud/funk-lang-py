    ...
    impl Lexer {
                ...
                if current.unwrap().is_whitespace() {
                    if current.unwrap() == &'\n' {
                        ...
                        tokens.push(Token {
                            ...
                            value: String::from(*current.unwrap()),
                            line: self.line,
        ...
        fn prase_number(&mut self, iter: &mut Peekable<Iter<char>>, first: &char) -> Token {
            ...
            while iter.peek() != None
                && (iter.peek().unwrap().is_numeric() || iter.peek().unwrap() == &&'.')
            {
        ...
        fn parse_identifier(&mut self, iter: &mut Peekable<Iter<char>>, first: &char) -> Token {
            ...
            while iter.peek() != None
                && (iter.peek().unwrap().is_alphanumeric() || iter.peek().unwrap() == &&'_')
            {
