pub mod parser {
    ...
    impl Parser {
            ...
            if self.current_token == None {
                ...
                if typ_tkn != None {
                    if !typ_tkn.as_ref().unwrap().contains(&current_token.typ) {
                        panic!(
                            "Expected one of: {:?} on line {}, instead found {:?}",
                            typ_tkn.clone().unwrap(),
                            current_token.line,
                            current_token.typ
                        );
                    } else {
        ...
        fn parse_expr(&mut self) -> AST {
            ...
            if self.current_token != None
                && self.current_token.as_ref().unwrap().typ == TokenType::Operator
                && ["+".to_string(), "-".to_string(), "==".to_string()]
                    .contains(&self.current_token.as_ref().unwrap().value)
            {
        ...
        fn parse_factor(&mut self) -> AST {
            ...
            if current.typ == TokenType::Number {
                // Handle Numbers
                ...
                AST::Str { value: val }
            } else if current.typ == TokenType::LPar {
                // Handle Pars
                ...
                if self.current_token.as_ref().unwrap().typ == TokenType::LPar {
                    ...
                    AST::CallItem {
                        name: var.value,
                        call_type: String::from("VariableCall"),
                        args: None,
        ...
        fn parse_call(&mut self, name: Token) -> AST {
            ...
            AST::CallItem {
                name: name.value,
                call_type: String::from("FunctionCall"),
                args: Some(args),
                scope: String::from("Global"),
            }
        }
        ...
        fn parse_args(&mut self) -> Vec<Box<AST>> {
            ...
            while self.current_token != None {
                if [TokenType::RPar, TokenType::Newline, TokenType::Semi]
                    .contains(&self.current_token.as_ref().unwrap().typ)
                {
                    self.expect(
                        Some(vec![TokenType::RPar, TokenType::Newline, TokenType::Semi]),
                        None,
                    );
                    break;
                }
        ...
        fn parse_params(&mut self) -> Vec<Box<AST>> {
            ...
            while self.current_token != None {
                ...
                self.expect(Some(vec![TokenType::Identifier]), None);
                self.expect(Some(vec![TokenType::Comma, TokenType::RPar]), None);
                params.push(Box::new(AST::FunktionParameter {
                    name: param_name,
                    typ: param_typ,
                }))
            }
        ...
        fn parse_body(&mut self) -> Vec<Box<AST>> {
            ...
            while self.current_token != None
                && self.current_token.as_ref().unwrap().typ != TokenType::RCurl
            {
                self.skip_newlines();
                if self.current_token != None
                    && self.current_token.as_ref().unwrap().typ == TokenType::RCurl
        ...
        fn parse_conditional(&mut self) -> AST {
            ...
            if self.current_token != None
                && self.current_token.as_ref().unwrap().typ == TokenType::Keyword
                && ["elseif".to_string(), "else".to_string()]
                    .contains(&self.current_token.clone().unwrap().value)
            {
